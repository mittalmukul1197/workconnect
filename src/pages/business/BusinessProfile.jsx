  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl text-slate-900 pb-12 mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-slate-950 via-indigo-950 to-slate-950 text-white shadow-xl border-2 animate-shining-border">
        ...
        >
          {t('businessProfile.viewContracts')}
        </Button>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 text-indigo-900 text-xs font-semibold flex items-center gap-3 shadow-xs">
        ...
        <p className="text-[11px] leading-relaxed text-indigo-950">
          <strong className="font-extrabold text-indigo-900">{t('businessProfile.authNotice')}</strong> {t('businessProfile.authDesc')}
        </p>
      </div>
      ...
      <Card borderVariant="indigo" className="p-6 sm:p-8 space-y-6 bg-white shadow-md rounded-3xl">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <h2 className="text-base font-extrabold text-slate-900 flex items-center gap-2">
            ...
            <span>{t('businessProfile.title')}</span>
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('businessProfile.registeredName')}</span>
            <p className="font-extrabold text-slate-900 text-sm">{business.name}</p>
          </div>

          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('businessProfile.representative')}</span>
            <p className="font-extrabold text-indigo-700 text-sm">{business.contactPerson}</p>
          </div>

          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('businessProfile.sector')}</span>
            <p className="font-extrabold text-slate-900">{business.industry}</p>
          </div>

          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('businessProfile.officialPhone')}</span>
            <p className="font-extrabold text-slate-900">{business.phone}</p>
          </div>

          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('businessProfile.corporateEmail')}</span>
            <p className="font-extrabold text-slate-900">{business.email}</p>
          </div>

          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('businessProfile.escrowBalance')}</span>
          ...
          <div className="sm:col-span-2 p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('businessProfile.factoryAddress')}</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
          <div className="p-4 rounded-2xl bg-slate-900 text-white space-y-2">
            <span className="text-[10px] text-emerald-400 font-extrabold uppercase tracking-wider block">{t('businessProfile.escrowProtection')}</span>
            <h4 className="font-extrabold text-sm text-white">{t('businessProfile.antiScam')}</h4>
            <p className="text-xs text-slate-300">
              {t('businessProfile.escrowDesc')}
            </p>
          </div>

          <div className="p-4 rounded-2xl bg-gradient-to-r from-indigo-900 to-purple-900 text-white space-y-2">
            <span className="text-[10px] text-indigo-300 font-extrabold uppercase tracking-wider block">{t('businessProfile.workforcePool')}</span>
            <h4 className="font-extrabold text-sm text-white">{t('businessProfile.activePool')} ({business.activeContractorsCount})</h4>
            <p className="text-xs text-indigo-200">
              {t('businessProfile.poolDesc')}
            </p>
          </div>
        </div>
      </Card>
